# app/core/etl/aggregate.py
"""
AGGREGATE MODULE - Generate insights from processed data

Purpose:
    1. Calculate spending patterns and trends
    2. Generate financial reports
    3. Provide dashboard analytics
    4. Track financial goals and budgets

Data Flow:
    load.py (optimized data) → calculate_metrics() → generate_insights() → analytics_dashboard
                                                                          ↓
                                                                    Financial insights
...
# ============================================================================
# STEP 1: BASIC SPENDING ANALYTICS
# ============================================================================
...
async def get_user_spending_by_category(
    ...
    ).group_by(models.Transaction.category).order_by(desc('total_amount'))
...
async def get_monthly_spending_trend(
    ...
    Great for line charts showing spending patterns.
    ...
    stmt = select(
        ...
        func.coalesce(func.sum(models.Transaction.amount), 0).label('monthly_spending'),
        func.count(models.Transaction.id).label('transaction_count')
    ).where(
        and_(
            models.Transaction.owner_id == user_id,
...
async def get_top_merchants(
    ...
    ).group_by(models.Transaction.merchant).order_by(desc('total_amount')).limit(limit)
...
async def get_income_analysis(
    ...
    category_stmt = select(
        models.Transaction.category,
        func.coalesce(func.sum(models.Transaction.amount), 0).label('category_amount'),
    ...
    ).group_by(models.Transaction.category).order_by(desc('category_amount'))
    ...
    total_income = float(getattr(income_row, 'total_income', 0)) if income_row and getattr(income_row, 'total_income', None) else 0
    ...
    return {
        ...
        "total_transactions": getattr(income_row, 'transaction_count', 0) if income_row else 0
    }


# ============================================================================
# STEP 3: SAVINGS AND FINANCIAL HEALTH
# ============================================================================
...
async def calculate_savings_rate(
    ...
    total_expenses = abs(float(expense_value)) if expense_value is not None else 0  # Make positive
    ...
    if savings_rate >= 20:
        ...
        recommendation = "Focus on increasing income or reducing expenses significantly."
...
async def create_budget_recommendations(
    ...
    current_spending = await get_user_spending_by_category(user_id, start_date, end_date, db)
    ...
    budget_rules = {
        ...
        "Food & Restaurants": {"percentage": 15, "type": "needs"},  # Part wants, part needs
...
def generate_category_recommendation(
    ...
    if over_percentage > 50:
        ...
        return f"⚠️ Urgent: You're spending {over_percentage:.0f}% more than recommended for {category}. {recommendations[0]}"
    else:
...
def generate_financial_insights(
    ...
    if income_analysis["income_by_category"]:
        salary_income = next(
            (cat for cat in income_analysis["income_by_category"] 
...
async def get_dashboard(
    ...
    dashboard_data = await get_financial_dashboard(
...
EXAMPLE 2: Get spending by category
====================================
...
async def get_spending_analytics(
    ...
    spending = await get_user_spending_by_category(
        user_id=current_user.id,
...
EXAMPLE 3: Get budget recommendations
======================================

@router.get("/analytics/budget-recommendations")
async def get_budget_recommendations(
    ...
    recommendations = await create_budget_recommendations(
        user_id=current_user.id,
        start_date=start_date,
        end_date=end_date,
        db=db
...
EXAMPLE 4: Financial health check
=================================
...
async def financial_health_check(
    ...
    savings_rate = await calculate_savings_rate(
        user_id=current_user.id,
        start_date=start_date,
        end_date=end_date,
        db=db
